...
import { X, Upload, Check, Image as ImageIcon, Monitor, Tablet, Smartphone } from 'lucide-react';
import { useStore, BACKGROUND_PRESETS } from '../store/useStore';
...
export const ThemePicker: React.FC<ThemePickerProps> = ({ onClose }) => {
  const { backgroundImage, setBackgroundImage, showToast, language } = useStore();
  ...
  const renderGrid = (images: string[], label: string, Icon: any) => (
    <div className="mb-6">
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={{ scale: 1, y: 0 }}
        exit={{ scale: 0.9, y: 20 }}
        className="glass-panel w-full max-w-3xl rounded-xl border border-neon-cyan/30 overflow-hidden flex flex-col max-h-[80vh] bg-cozy-dark"
      >
        {/* Header */}
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-black/40">
          <div className="flex items-center gap-3">
             <ImageIcon className="text-neon-cyan" size={24} />
             <h2 className="text-2xl font-display font-bold text-white tracking-widest">{t.theme_title}</h2>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="mb-8">
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4">{t.theme_upload_title}</h3>
            <button 
              type="button"
            ...
            >
              ...
              <span className="text-gray-400 group-hover:text-white font-display text-sm">{t.theme_upload_desc}</span>
              <span className="text-xs text-gray-600 mt-1 font-mono">{t.theme_upload_sub}</span>
            </button>
          </div>

          {/* Presets Sections */}
          {renderGrid(BACKGROUND_PRESETS.desktop, t.theme_desktop, Monitor)}
          {renderGrid(BACKGROUND_PRESETS.tablet, t.theme_tablet, Tablet)}
          {renderGrid(BACKGROUND_PRESETS.mobile, t.theme_mobile, Smartphone)}
