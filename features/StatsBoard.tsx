...
export const StatsBoard: React.FC = () => {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto px-4 md:px-0">
      ...
      {/* Controls Toolbar */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-stone-900/50 p-4 rounded-xl border border-white/5 backdrop-blur-md">
         <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 md:gap-4 w-full md:w-auto">
           ...
           <div className="flex bg-stone-950 rounded-lg p-1 border border-white/5 w-full sm:w-auto justify-between sm:justify-start">
             <button 
               onClick={() => setRange(7)}
               className={`flex-1 sm:flex-none px-4 py-1.5 text-[10px] font-bold rounded-md transition-all uppercase tracking-wider ${range === 7 ? 'bg-white/10 text-stone-200' : 'text-stone-600 hover:text-stone-300'}`}
             >
               7 Days
             </button>
             <button 
               onClick={() => setRange(30)}
               className={`flex-1 sm:flex-none px-4 py-1.5 text-[10px] font-bold rounded-md transition-all uppercase tracking-wider ${range === 30 ? 'bg-white/10 text-stone-200' : 'text-stone-600 hover:text-stone-300'}`}
             >
               30 Days
             </button>
           </div>
         ...
         <div className="flex gap-2 w-full md:w-auto justify-end">
           <CyberButton size="sm" variant="ghost" onClick={handleRefresh} title="Refresh Data">
             <RefreshCw size={16} className={loading ? 'animate-spin' : ''} />
           </CyberButton>
           <CyberButton size="sm" variant="secondary" onClick={handleExport} title="Download CSV" className="flex-1 md:flex-none justify-center">
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <CyberCard className="flex flex-col items-center justify-center py-10 relative overflow-hidden">
              <span className="text-stone-500 font-sans text-xs uppercase tracking-[0.2em] mb-3 z-10">Total Focus Time</span>
              <span className="text-5xl md:text-6xl font-serif text-stone-200 text-center z-10">
            ...
            <CyberCard className="flex flex-col items-center justify-center py-10 relative overflow-hidden">
              ...
              <span className="text-5xl md:text-6xl font-serif text-cozy-amber z-10">
                {totalTasks}
              </span>
            </CyberCard>
          </div>

          <CyberCard className="h-[350px] md:h-[450px]">
